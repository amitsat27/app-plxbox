/**
 * Bills Screen — stack navigation (from Sections), not a tab
 */
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: isDark ? '#000' : '#F2F2F7' }]}>
      <View style={[styles.header, { paddingTop: Math.max(insets.top, 4) }]}>
        ...
        <Text style={[styles.sub, { color: scheme.textTertiary }]}>{allBills.length} bills</Text>
        <Text style={[styles.title, { color: scheme.textPrimary }]}>All Bills</Text>
      </View>
      ...
      <FlatList
        ...
        ListEmptyComponent={<Text style={[styles.empty, { color: scheme.textTertiary }]}>No bills yet.</Text>}
      />
    </SafeAreaView>
  );
}
