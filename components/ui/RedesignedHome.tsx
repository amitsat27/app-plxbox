import React, { useEffect, useState, useMemo, useRef, ErrorInfo } from 'react';
import {
  StyleSheet,
  View,
  ScrollView,
...
class ChartsErrorBoundary extends React.Component<{ children: React.ReactNode }, ErrorBoundaryState> {
    ...
    if (this.state.hasError) {
      return (
        <View style={{ padding: 20, alignItems: 'center' }}>
          <Text style={{ color: Colors.textSecondary }}> Charts unavailable </Text>
        </View>
      );
    }
...
export const RedesignedHome: React.FC = () => {
  ...
  const billData = useMemo(() => {
    const categories: Record<string, { amount: number; count: number; color: string; label: string; icon: any }> = {
      electric: { amount: 0, count: 0, color: Colors.categoryElectric, label: 'Electric', icon: Zap },
      water: { amount: 0, count: 0, color: Colors.categoryWater, label: 'Water', icon: Droplet },
      gas: { amount: 0, count: 0, color: Colors.categoryGas, label: 'Gas', icon: Flame },
      wifi: { amount: 0, count: 0, color: Colors.categoryWifi, label: 'WiFi', icon: Wifi },
      property: { amount: 0, count: 0, color: Colors.categoryProperty, label: 'Property', icon: Home },
    };
  ...
  const totalBillsAmount = billData.reduce((sum, cat) => sum + (isFinite(cat.amount) ? cat.amount : 0), 0);
  const totalBillsCount = billData.reduce((sum, cat) => sum + cat.count, 0);
  const pendingBills = bills.filter(b => b.status === 'pending');
  const pendingAmount = pendingBills.reduce((sum, bill) => sum + (isFinite(bill.amount) ? bill.amount : 0), 0);
  const overdueBills = bills.filter(b => b.status === 'overdue');
  const overdueAmount = overdueBills.reduce((sum, bill) => sum + (isFinite(bill.amount) ? bill.amount : 0), 0);
  ...
  if (loading) {
    return (
      <View style={[styles.gradient, { backgroundColor: isDark ? Colors.backgroundDark : Colors.background }]}>
  ...
  const vehiclesSummary = vehicles.length > 0 ? {
    id: 'vehicles',
    title: 'Vehicles',
        ...
        <ScrollView
          ...
          refreshControl={
            <RefreshControl
              ...
              progressBackgroundColor={isDark ? Colors.surfaceDark : Colors.surface}
            />
          }
